...
object CalculatorApi {
    ...
    // ---------------- POST /gst/calculate ----------------
    fun gstCalculate(request: GstCalcRequest): String {
        return try {
            if (request.amount < 0) throw IllegalArgumentException("Amount cannot be negative")
            if (request.gstPercent < 0) throw IllegalArgumentException("GST rate cannot be negative")

            val rate = request.gstPercent / 100.0
            val (base, totalGst, total) = when (request.mode.uppercase()) {
                "ADD" -> {
                    val base = request.amount
                    val gst = base * rate
                    Triple(base, gst, base + gst)
                }
                "REMOVE" -> {
                    val total = request.amount
                    val base = total / (1.0 + rate)
                    val gst = total - base
                    Triple(base, gst, total)
                }
                else -> throw IllegalArgumentException("Unknown mode: ${request.mode}")
            }
            val half = totalGst / 2.0

            val response = GstCalcResponse(
                baseAmount = formatCurrency(base),
                cgst = formatCurrency(half),
                sgst = formatCurrency(half),
                totalGst = formatCurrency(totalGst),
                totalAmount = formatCurrency(total)
            )
            json.encodeToString(response)
        } catch (e: Exception) {
            errorJson("GST_CALC_ERROR", e.message ?: "Invalid GST calculation")
        }
    }
    ...
    /** Always shows exactly two decimal places - used for money amounts (GST). */
    private fun formatCurrency(value: Double): String {
        if (value.isNaN() || value.isInfinite()) throw ArithmeticException("Invalid amount")
